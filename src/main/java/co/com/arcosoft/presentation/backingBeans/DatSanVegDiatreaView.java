...
@ManagedBean
@ViewScoped
public class DatSanVegDiatreaView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(DatSanVegDiatreaView.class);
	...
	private DatSanVegDTO selectedDatSanVegDiatrea;
	...
	// Campos Informacion lote/suerte

	private double hectareaNetaN4 = 0D;
	private String variedadN4 = "";
	private String fechaCorteN4;
	private String etapaN4 = "";
	private double edadN4 = 0D;
	private boolean desactivarBoton = true;
	private String estadoIntensidadInfDiatrea = "";
	private int activeTapIndex = 0;

	public DatSanVegDiatreaView() {
		super();
	}
	...
	public String action_clear() {
		...
		selectedDatSanVegDiatrea = null;
		PrimeFaces.current().resetInputs(":dialogDatSanVegDiatrea :frm");
		activeTapIndex = 0;
		...
		/*
		 * if (txtVariable11 != null) { txtVariable11.setValue(null);
		 * txtVariable11.setDisabled(false); } if (txtVariable12 != null) {
		 * txtVariable12.setValue(null); txtVariable12.setDisabled(false); } if
		 * (txtVariable13 != null) { txtVariable13.setValue(null);
		 * txtVariable13.setDisabled(false); } if (txtVariable14 != null) {
		 * txtVariable14.setValue(null); txtVariable14.setDisabled(false); } if
		 * (txtVariable15 != null) { txtVariable15.setValue(null);
		 * txtVariable15.setDisabled(false); } if (txtVariable16 != null) {
		 * txtVariable16.setValue(null); txtVariable16.setDisabled(false); } if
		 * (txtVariable17 != null) { txtVariable17.setValue(null);
		 * txtVariable17.setDisabled(false); } if (txtVariable18 != null) {
		 * txtVariable18.setValue(null); txtVariable18.setDisabled(true); } if
		 * (txtVariable19 != null) { txtVariable19.setValue(null);
		 * txtVariable19.setDisabled(true); } if (txtVariable20 != null) {
		 * txtVariable20.setValue(null); txtVariable20.setDisabled(true); } if
		 * (txtVariable21 != null) { txtVariable21.setValue(null);
		 * txtVariable21.setDisabled(true); } if (txtVariable22 != null) {
		 * txtVariable22.setValue(null); txtVariable22.setDisabled(true); } if
		 * (txtVariable23 != null) { txtVariable23.setValue(null);
		 * txtVariable23.setDisabled(true); } if (txtVariable24 != null) {
		 * txtVariable24.setValue(null); txtVariable24.setDisabled(true); }
		 */
	...
	public MapModel getSimpleModel() {
		...
		if ((Float) txtLatitud.getValue() != null && (Float) txtLongitud.getValue() != null) {
			...
			// Basic marker
			simpleModel.addOverlay(new Marker(coord1, "Casos"));
		} else {
			// Shared coordinates
	...
	public void listener_calcular_IntensidadInfDiatrea() {
		...
		Double entrenudosDiatrea = FacesUtils.checkDouble(getTxtVariable3());
		Double totalEntrenudos = FacesUtils.checkDouble(getTxtVariable2());

		if (entrenudosDiatrea != null && (totalEntrenudos != null || totalEntrenudos != 0)) {
			result = (entrenudosDiatrea / totalEntrenudos) * 100;
			String format = decimalFormat.format(result);
			txtVariable7.setValue(format);
			evaluarEstadoIntensidadInfDiatrea(result);
		} else {
			result = 0;
			txtVariable7.setValue(result);
		}

	}

	public void evaluarEstadoIntensidadInfDiatrea(double valor) {

		if (valor >= 0 && valor <= 2.0) {
			estadoIntensidadInfDiatrea = "Sano";
		} else if (valor >= 2.1 && valor <= 5.0) {
			estadoIntensidadInfDiatrea = "Dañado";
		} else if (valor >= 5.1) {
			estadoIntensidadInfDiatrea = "Muy dañado";
		}

	}

	@SuppressWarnings("null")
	public void listener_calcular_Por_InfesDiatrea() {
		...
		Double canasDiatrea = FacesUtils.checkDouble(getTxtVariable5());
		Double canasAnalizadas = FacesUtils.checkDouble(getTxtVariable1());

		if (canasDiatrea != null && (canasAnalizadas != null || canasAnalizadas != 0)) {
			result = (canasDiatrea / canasAnalizadas) * 100;
			String format = decimalFormat.format(result);
			txtVariable8.setValue(format);
		} else {
			result = 0;
			txtVariable8.setValue(result);
		}

	}

	@SuppressWarnings("null")
	public void listener_calcular_Int_infesValentina() {
		...
		Double entrenudosValentina = FacesUtils.checkDouble(getTxtVariable4());
		Double totalEntrenudos = FacesUtils.checkDouble(getTxtVariable2());

		if (entrenudosValentina != null && (totalEntrenudos != null || totalEntrenudos != 0)) {
			result = (entrenudosValentina / totalEntrenudos) * 100;
			String format = decimalFormat.format(result);
			txtVariable9.setValue(format);
		} else {
			result = 0;
			txtVariable9.setValue(result);
		}

	}

	@SuppressWarnings("null")
	public void listener_calcular_Por_InfesValentina() {
		...
		Double canasValentina = FacesUtils.checkDouble(getTxtVariable6());
		Double canasAnalizadas = FacesUtils.checkDouble(getTxtVariable1());

		if (canasValentina != null && (canasAnalizadas != null || canasAnalizadas != 0)) {
			result = (canasValentina / canasAnalizadas) * 100;
			String format = decimalFormat.format(result);
			txtVariable10.setValue(format);
		} else {
			result = 0;
			txtVariable10.setValue(result);
		}
	...
	public String action_edit(ActionEvent evt) {
		selectedDatSanVegDiatrea = (DatSanVegDTO) (evt.getComponent().getAttributes().get("selectedDatSanVegDiatrea"));

		PrimeFaces.current().resetInputs(":dialogDatSanVegDiatrea");

		try {

			Long consecutivo = selectedDatSanVegDiatrea.getConsecutivo();
			Object[] condicion = { "consecutivo", true, consecutivo, "=" };
			List<DatSanVeg> lista = (consecutivo != null)
					? businessDelegatorView.findByCriteriaInDatSanVeg(condicion, null, null) : null;
			...
			if (lista != null && lista.size() > 0) {
				...
				txtVariable7.setDisabled(false);
				txtVariable7.setValue(entity.getVariable7());
				evaluarEstadoIntensidadInfDiatrea(entity.getVariable7().doubleValue());
				txtVariable8.setDisabled(false);
				txtVariable8.setValue(entity.getVariable8());
				txtVariable9.setDisabled(false);
				txtVariable9.setValue(entity.getVariable9());
				...
				//getInformacionLoteSuerte(entity.getEdadNivel4().doubleValue());
				txtDatSanVegId.setValue(entity.getDatSanVegId());
				txtDatSanVegId.setDisabled(true);
	...
	public String action_save() {
		try {
			if ((selectedDatSanVegDiatrea == null) && (entity == null)) {
				action_create();
			} else {
				action_modify();
			}
	...
	public String action_create() {
		try {
			...
			// Variables digitadas
			entity.setVariable1(FacesUtils.checkDouble(txtVariable1)); // Número
			// de
			// cañas
			// analizadas
			entity.setVariable2(FacesUtils.checkDouble(txtVariable2)); // Total
			// de
			// entrenudos
			entity.setVariable3(FacesUtils.checkDouble(txtVariable3)); // Total
			// entrenudos
			// con
			// diatrea:
			entity.setVariable4(FacesUtils.checkDouble(txtVariable4)); // Total
			// entrenudos
			// con
			// valentina:
			entity.setVariable5(FacesUtils.checkDouble(txtVariable5)); // Cañas
			// con
			// diatrea:
			entity.setVariable6(FacesUtils.checkDouble(txtVariable6)); // Cañas
			// con
			// valentina:

			// Variables calculadas
			entity.setVariable7(FacesUtils.checkDouble(txtVariable7)); // Intensidad
			// de
			// infestación
			// diatrea:
			...
			// Infestación
			// diatrea:
			entity.setVariable9(FacesUtils.checkDouble(txtVariable9)); // Intensidad
			// infestación
			// valentina
			...
			/**
			 * Calcular la edad actual según la fecha de registro del analisis
			 * txtFechaAnalisis
			 */
			...
			entity.setVariedNivel4(nivel4.getVariedad().longValue());

			entity.setEdadNivel4((businessDelegatorView.calcularEdadLote(FacesUtils.checkDate(txtFechaAnalisis),
	...
	public String action_modify() {
		try {
			if (entity == null) {
				Long datSanVegId = new Long(selectedDatSanVegDiatrea.getDatSanVegId());
				entity = businessDelegatorView.getDatSanVeg(datSanVegId);
			}
			Date date = new Date();
			Long compania = new Long(getCompaniaIdSession());
	...
	/**
	 * 
	 * @param fechaultimocorte
	 * @param fechaSiembra
	 * @param cultivo
	 * @param fechaEjecucion:
	 *            es equivalente a la txtFechaAnalisis
	 * @return
	 * @throws Exception
	 */
	public String action_delete_datatable(ActionEvent evt) {
		try {
			selectedDatSanVegDiatrea = (DatSanVegDTO) (evt.getComponent().getAttributes()
					.get("selectedDatSanVegDiatrea"));
	...
	public String actionDeleteDataTableEditable(ActionEvent evt) {
		try {
			selectedDatSanVegDiatrea = (DatSanVegDTO) (evt.getComponent().getAttributes().get("selectedDatSanVeg"));

			Long datSanVegId = new Long(selectedDatSanVegDiatrea.getDatSanVegId());
			entity = businessDelegatorView.getDatSanVeg(datSanVegId);
			businessDelegatorView.deleteDatSanVeg(entity);
			data.remove(selectedDatSanVegDiatrea);
			FacesUtils.addInfoMessage(ZMessManager.ENTITY_SUCCESFULLYDELETED);
			action_clear();
		} catch (Exception e) {
			FacesUtils.addErrorMessage(e.getMessage());
	...
	public void action_formacionnLoteSuerte() {

		getInformacionLoteSuerte(0.0);
	}

	public void getInformacionLoteSuerte(double edadAna) {

		Long nivel4Id = null;

		GregorianCalendar calendarioInfoLote = new GregorianCalendar();

		if (txtNivel4Id_Nivel4 != null && txtNivel4Id_Nivel4.getValue() != null
				&& !txtNivel4Id_Nivel4.getValue().equals("")) {
			...
			try {
				...
				Nivel4 nivel4 = businessDelegatorView.getNivel4(nivel4Id);
				Variedad variedad = businessDelegatorView.getVariedad(nivel4.getVariedad().longValue());
				Etapa etapa = businessDelegatorView.getEtapa(nivel4.getEtapa().getEtapaId().longValue());

				hectareaNetaN4 = nivel4.getAreaNeta().doubleValue();

				if (variedad != null) {
					variedadN4 = variedad.getNombre().toString();
				}

				calendarioInfoLote.setTime(nivel4.getFechaUltimoCorte());
				int diaCosecha = calendarioInfoLote.get(java.util.Calendar.DAY_OF_MONTH);
				int mesCosecha = calendarioInfoLote.get(java.util.Calendar.MONTH);
				int anoCosecha = calendarioInfoLote.get(java.util.Calendar.YEAR);

				calendarioInfoLote.set(anoCosecha, mesCosecha, diaCosecha);

				DateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");

				fechaCorteN4 = dateFormat.format(new java.sql.Date(calendarioInfoLote.getTimeInMillis()));

				if (etapa != null) {
					etapaN4 = etapa.getNombre().toString();
				}

				edadN4 = (businessDelegatorView.calcularEdadLote(FacesUtils.checkDate(txtFechaAnalisis),
						FacesUtils.checkLong(txtNivel4Id_Nivel4)));
	...
	public SelectItem[] getSelectTipoAnalisis() {
		...
		if (tipoAnalisis == null || tipoAnalisis.size() == 0) {
			...
			try {
				...
				// Criteria
				Object[] condicion = { "estado", true, "A", "=", "anaSanVegId", true, 5, "=" };
	...
	public List<DatSanVegDTO> getData() {
		try {
			...
			if (data == null) {
				...
				data = businessDelegatorView.getDataDatSanVeg(5L);
	...
	public DatSanVegDTO getselectedDatSanVegDiatrea() {
		return selectedDatSanVegDiatrea;
	}

	public void setselectedDatSanVegDiatrea(DatSanVegDTO datSanVeg) {
		this.selectedDatSanVegDiatrea = datSanVeg;
	}
	...
	public double getHectareaNetaN4() {
		return hectareaNetaN4;
	}

	public String getVariedadN4() {
		return variedadN4;
	}

	public String getFechaCorteN4() {
		return fechaCorteN4;
	}

	public String getEtapaN4() {
		return etapaN4;
	}

	public double getEdadN4() {
		return edadN4;
	}
	...
	public void setHectareaNetaN4(double hectareaNetaN4) {
		this.hectareaNetaN4 = hectareaNetaN4;
	}

	public void setVariedadN4(String variedadN4) {
		this.variedadN4 = variedadN4;
	}

	public void setFechaCorteN4(String fechaCorteN4) {
		this.fechaCorteN4 = fechaCorteN4;
	}

	public void setEtapaN4(String etapaN4) {
		this.etapaN4 = etapaN4;
	}

	public void setEdadN4(double edadN4) {
		this.edadN4 = edadN4;
	}
	...
	public String getEstadoIntensidadInfDiatrea() {
		return estadoIntensidadInfDiatrea;
	}

	public void setEstadoIntensidadInfDiatrea(String estadoIntensidadInfDiatrea) {
		this.estadoIntensidadInfDiatrea = estadoIntensidadInfDiatrea;
	}
