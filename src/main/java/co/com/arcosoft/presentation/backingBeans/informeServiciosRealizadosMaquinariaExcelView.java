...
@ManagedBean
@ViewScoped
public class informeServiciosRealizadosMaquinariaExcelView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(informeServiciosRealizadosMaquinariaExcelView.class);
	...
	public informeServiciosRealizadosMaquinariaExcelView() {
		super();
	}
		...
		if (compania != null) {
			try {
				...
				filename = servletContext.getRealPath("") + File.separator + "informes" + File.separator
						+ "MttoServiciosRealizadosMaquinariaExcel.xlsx";

				data = businessDelegatorView.consultarServRealizadosEquipo(compania, fechaInicial, fechaFinal,
						zonasSeleccionadas, flagZona, fincasSeleccionadas, flagFinca, bloquesSeleccionadas, flagBloque,
						lotesSeleccionadas, flagLote, laboresSeleccionadas, flagLabor, "",
						1l, grupoLaboresSeleccionadas, flagGrupoLabor, "",
						1l, "", 1l, "",
						1l, equiposSeleccionadas, flagEquipo);
				try {
					...
					if (data != null) {
						...
						cell.setCellValue("IngresoTotal");
						...
						file = new DefaultStreamedContent(stream, "application/xlsx", "MttoServiciosRealizadosMaquinariaExcel.xlsx");
	...
	public List<HorasMaquinaDetalladoDTO> getData() {
		try {
			if (data == null) {
				data = businessDelegatorView.consultarServRealizadosEquipo(null, null, null, null, null, null,
