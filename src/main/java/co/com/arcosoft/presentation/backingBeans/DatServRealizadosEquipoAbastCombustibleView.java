...
@ManagedBean
@ViewScoped
public class DatServRealizadosEquipoAbastCombustibleView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(DatServRealizadosEquipoAbastCombustibleView.class);
	...
	public DatServRealizadosEquipoAbastCombustibleView() {
		super();
	}
			...
			if (lista != null && lista.size() > 0) {
				...
				if (equipoId != null && !equipoId.toString().isEmpty()) {
					...
					if (equipo.getProductoId() != null) {
						...
						//txtProducto.setValue(productoId);
						//txtProducto.setDisabled(false);
						txtAlmacenId_Almacen2.setDisabled(false);
						txtHorometroAbastecimiento.setDisabled(false);
...
/*
	public String action_create() {
		try {
			...
			java.sql.Date fechaMinima = new java.sql.Date(calendario2.getTimeInMillis());

			if (fechaPin.before(fecha) && fechaMinima.before(fechaPin)) {
				if (dataDetPrecioProductos != null && dataDetPrecioProductos.size() > 0) {
					businessDelegatorView.saveDatServRealizadosEquipo(entity, null, dataDetPrecioProductos);
...
*/
	public String action_modify() {
		try {
			...
			if (fechaPin.before(fecha) && fechaMinima.before(fechaPin)) {
				if (dataDetPrecioProductos != null && dataDetPrecioProductos.size() > 0) {
					businessDelegatorView.updateDatServRealizadosEquipo(entity, null, dataDetPrecioProductos);
					...
					FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
							"Upps!, Falta registrar y agregar el detalle del producto " + "Verifique que los campos "
									+ "Bodega, Producto, Hórometro de abastecimiento, cantidad, "
									+ "" + "tengan valores. ",
							""));
				}
						...
						if (horometroIni != null && horometroFin != null) {
							if (horometroIni <= horometroFin.doubleValue()) {
								...
								if (calculoHorasHorometro <= maxDifHorometro) {
									...
									if (tipoUm != null) {
										if (tipoUm.equals("HR")) {
											totalHorometro = FacesUtils.checkDouble(txtCantidad);
											totalHoras = FacesUtils.checkDouble(txtCantidad);
										} else {
									...
									if (dataServDet.size() > 0) {
										...
										for (DatServRealizadosEquipoDetDTO d : dataServDet) {
											...
											if (d.getPersEmpr().getPersEmprId().longValue() == persEmpr.getPersEmprId()
													.longValue()
													...
													&& d.getNivel2ClientesId().getNivel2ClientesmqId()
															.longValue() == nivel2Clientes.getNivel2ClientesmqId()
																	.longValue()
													// && d.getNivel4ClientesId().getNivel4ClientesmqId().longValue() ==
													// nivel4Clientes
													// .getNivel4ClientesmqId().longValue()

													&& d.getHorometroInicial() == horometroIni
													&& d.getTurno().equals(turno)) {

												existeProducto = true;

												FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(
														FacesMessage.SEVERITY_WARN, "Upps!",
														"La Labor/Cliente/Hda/Lote/Unidad de mÃ©dida "
																+ d.getLaborId_Labor().getNombre() + " "
																+ d.getNivel2ClientesId().getNombre() + " "
																+ d.getNombreNivel4()
	...
	public void action_agregarCombustible() throws Exception {

		
		Double horometroAbastecimiento = FacesUtils.checkDouble(txtHorometroAbastecimiento);
		Double cantidadAbastecimiento = FacesUtils.checkDouble(txtCantidadCombustible);

		if (cantidadAbastecimiento != null && cantidadAbastecimiento > 0 && txtFechaRegistro.getValue() != null
				 && txtAlmacenId_Almacen2.getValue() != null) {
...
/*
			...
			String codTrabajador = trabajador.getNombre();
*/			
			String infoAdicionalCombustible =   FacesUtils.checkString(txtInfoAdicionalCombustible);
			Long equipoId = FacesUtils.checkLong(txtEquipoId_Equipo);
			Equipo equipo = null;
			Long idProducto = 0L;
			Long idUdadMed = 0L;
			...
			if (txtEquipoId_Equipo.getValue() != null) {
				equipo = businessDelegatorView.getEquipo(equipoId);
				idProducto = equipo.getProductoId();
			}
			...
			Double cantidadCombustible = FacesUtils.checkDouble(txtCantidadCombustible);
			horometroAbastecimiento = FacesUtils.checkDouble(txtHorometroAbastecimiento);
