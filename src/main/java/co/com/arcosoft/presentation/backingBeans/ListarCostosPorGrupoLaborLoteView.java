...
@ManagedBean
@ViewScoped
public class ListarCostosPorGrupoLaborLoteView implements Serializable {
	private static final long serialVersionUID = 1L;
	@SuppressWarnings("unused")
	private static final Logger log = LoggerFactory.getLogger(ListarCostosPorGrupoLaborLoteView.class);
	...
	public ListarCostosPorGrupoLaborLoteView() {
		super();
	}
	...
	public void listarPlanillaNominaDetallada() {
		try {
			...
			if (compania != null && fechaInicial != null && fechaFinal != null) {
				...
				dataDetalle = businessDelegator2View.pr_costos_resumen_labor_grupo_lotes_cosechados(compania, fechaInicial, fechaFinal, "0", 1L, 
					haciendasSeleccionadas, flagHacienda, "0", 1L, lotesSeleccionados, flagLote, "0", 1L, 0L, "0", "0", 1L);
			}
			...
			if (compania != null) {
				try {
					...
					filename = servletContext.getRealPath("") + File.separator + "informes" + File.separator
							+ "CostosPorGrupoLaborLote.xlsx";

					data = businessDelegator2View.pr_costos_resumen_labor_grupo_lotes_cosechados(compania, fechaInicial, fechaFinal, "0", 1L, 
						haciendasSeleccionadas, flagHacienda, "0", 1L, lotesSeleccionados, flagLote, "0", 1L, 0L, "0", "0", 1L);
					...
					try {
						...
						if (data != null) {
							...
							cell = row.createCell(0); cell.setCellStyle(style2); cell.setCellValue("CODIGO");
							cell = row.createCell(1); cell.setCellStyle(style2); cell.setCellValue("CONCEPTO");
							cell = row.createCell(2); cell.setCellStyle(style2); cell.setCellValue("CENTRO");
							cell = row.createCell(3); cell.setCellStyle(style2); cell.setCellValue("COSTO TOTAL");
							cell = row.createCell(4); cell.setCellStyle(style2); cell.setCellValue("COSTO/AREA");
							...
							for (CostosTotalesDTO ndDTO : data) {
								...
								cell = row.createCell(0); cell.setCellValue(ndDTO.getCodLabor());
								cell = row.createCell(1); cell.setCellValue(ndDTO.getNombreLabor());
								cell = row.createCell(2); cell.setCellValue(ndDTO.getNombreGrupoLabor());
								cell = row.createCell(3); cell.setCellValue(ndDTO.getCostoTotal().doubleValue());
								cell = row.createCell(4); cell.setCellValue(ndDTO.getCostoSobreArea().doubleValue());
							...
							file = new DefaultStreamedContent(stream, "application/xlsx", "CostosPorGrupoLaborLote.xlsx");
