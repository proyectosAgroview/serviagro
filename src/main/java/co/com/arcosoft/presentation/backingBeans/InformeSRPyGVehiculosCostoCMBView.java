...
@ManagedBean
@ViewScoped
public class InformeSRPyGVehiculosCostoCMBView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(InformeSRPyGVehiculosCostoCMBView.class);
	...
	private InputNumber txtKilometros;
	private InputNumber txtKmGln;
	private InputNumber txtCostoGln;
	...
	public InformeSRPyGVehiculosCostoCMBView() {
		super();
	}
	...
	public List<CategoriaEquipo> getCategoriaEquipos() {
		...
		if (categoriaEquipos == null || categoriaEquipos.size() == 0) {
			...
			try {
				Object[] condicion = { "estado", true, "A", "=", 
						 "funcion", true, "Motobomba/Electrogeno", "<>",
						"funcion", true, "Implemento", "<>",
						"funcion", true, "Remolques/Vagones", "<>",
						"funcion", true, "Herramienta", "<>",
						"funcion", true, "Otros", "<>",
						"funcion", true, "Tractor", "<>"
						
				};
				categoriaEquipos = businessDelegatorView.findByCriteriaInCategoriaEquipo(condicion, null, null);
	...
	public void consultaPyGVehiculos() {
		try {
			...
			if(compania != null && fechaInicial != null && fechaFinal != null){
				...
				data = businessDelegator2View.pr_pyg_equipo(compania,  fechaInicial, fechaFinal, 
					 equiposSeleccionadas, flagEquipo,categoriaEquiposSeleccionadas, flagEquipoCategoriaEquipos, estadoServicio,modelo);
			
		       if(data !=null && data.size()>=0) {
		        for( ConsultaServiciosRealizadosMaquinariaDTO data1 : data) {
		        	cantidad += data1.getCantidadPago().doubleValue();
		        	kilometros += data1.getKilometros().doubleValue();
		        	costoTotal += data1.getCosto_combustible().doubleValue();
			...
			txtKilometros.setValue(kilometros);
			txtCostoTotal.setValue(costoTotal);
			txtCombustible.setValue(combustible);
			
			if(kilometros >0	) {
				txtKmGln.setValue(	Math.round((kilometros/combustible) * 100.0) / 100.0);
				  txtCostoHr.setValue(Math.round((costoTotal/kilometros) * 100.0) / 100.0);
			}
			
			if(combustible >0	) {
				txtCostoGln.setValue(	Math.round((costoTotal/combustible) * 100.0) / 100.0);
	...
	public InputNumber getTxtKilometros() {
		return txtKilometros;
	}

	public void setTxtKilometros(InputNumber txtKilometros) {
	...
	public InputNumber getTxtKmGln() {
		return txtKmGln;
	}

	public void setTxtKmGln(InputNumber txtKmGln) {
		this.txtKmGln = txtKmGln;
	}

	public InputNumber getTxtCostoGln() {
		return txtCostoGln;
	}

	public void setTxtCostoGln(InputNumber txtCostoGln) {
		this.txtCostoGln = txtCostoGln;
	}
