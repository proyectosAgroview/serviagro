...
@ManagedBean
@ViewScoped
public class ListarLaboresPorInsumoView implements Serializable {
	private static final long serialVersionUID = 1L;
	@SuppressWarnings("unused")
	private static final Logger log = LoggerFactory.getLogger(ListarLaboresPorInsumoView.class);

	@ManagedProperty(value = "#{BusinessDelegatorView}")
	private IBusinessDelegatorView businessDelegatorView;
	private List<CostosInsumosDetalladoDTO> dataDetalle;
	...
	public ListarLaboresPorInsumoView() {
		super();
	}
	...
	public void listarPlanillaNominaDetallada() {
		try {
			...
			String productosSeleccionados = "0";
			if (selectedProducto != null && selectedProducto.size() > 0) {
				productosSeleccionados = selectedProducto.get(0);
				flagProducto = 0L;
				for (int i = 1; i < selectedProducto.size(); i++) {
					productosSeleccionados += "," + selectedProducto.get(i);
				}
			}
			...
			if (compania != null && fechaInicial != null && fechaFinal != null) {
				...
				dataDetalle = businessDelegatorView.consultarInformeCostosInsumosDetallado(compania, fechaInicial, fechaFinal,
					"0", 1L, haciendasSeleccionadas, flagHacienda, "0", 1L, lotesSeleccionados, flagLote, laboresSeleccionadas,
					flagLabor, "0", 1L, "0", 1L, "0", 1L, productosSeleccionados, flagProducto, 0L);
			}

			double totalUnidades = 0;
			double totalCosto = 0;
			if (dataDetalle != null && dataDetalle.size() >= 0) {
				for (CostosInsumosDetalladoDTO data1 : dataDetalle) {
		...
		try {
			...
			if (compania != null) {
				try {
					...
					List<CostosInsumosDetalladoDTO> data = null;
					...
					filename = servletContext.getRealPath("") + File.separator + "informes" + File.separator
							+ "LaboresPorInsumos.xlsx";

					data = businessDelegatorView.consultarInformeCostosInsumosDetallado(compania, fechaInicial, fechaFinal,
						"0", 1L, haciendasSeleccionadas, flagHacienda, "0", 1L, lotesSeleccionados, flagLote, laboresSeleccionadas,
						flagLabor, "0", 1L, "0", 1L, "0", 1L, productosSeleccionados, flagProducto, 0L);
					...
					try {
						...
						if (data != null) {
							...
							cell = row.createCell(3);  cell.setCellStyle(style2); cell.setCellValue("CANTIDAD APLICADA");
							cell = row.createCell(4);  cell.setCellStyle(style2); cell.setCellValue("VALOR UNITARIO");							
							cell = row.createCell(5);  cell.setCellStyle(style2); cell.setCellValue("SUBTOTAL");							
							cell = row.createCell(6);  cell.setCellStyle(style2); cell.setCellValue("HACIENDA");
							cell = row.createCell(7);  cell.setCellStyle(style2); cell.setCellValue("LOTE");							
							cell = row.createCell(8);  cell.setCellStyle(style2); cell.setCellValue("PRODUCTO");							
							cell = row.createCell(9);  cell.setCellStyle(style2); cell.setCellValue("AREA APLICADA");							
							cell = row.createCell(10); cell.setCellStyle(style2); cell.setCellValue("CONSECUTIVO");
							cell = row.createCell(11); cell.setCellStyle(style2); cell.setCellValue("ID");

							int pos_i = 8;

							for (CostosInsumosDetalladoDTO ndDTO : data) {
								...
								cell = row.createCell(1);  cell.setCellValue(ndDTO.getNombreLabor());
								cell = row.createCell(2);  cell.setCellValue(ndDTO.getCodUdadMedAplic());
								cell = row.createCell(3);  cell.setCellValue(ndDTO.getCantidadAplicada().doubleValue());
								cell = row.createCell(4);  cell.setCellValue(ndDTO.getValorUnit().doubleValue());								
								cell = row.createCell(5);  cell.setCellValue(ndDTO.getCostoTotal().doubleValue());								
								cell = row.createCell(6);  cell.setCellValue(ndDTO.getNomFinca());								
								cell = row.createCell(7);  cell.setCellValue(ndDTO.getNomLote());								
								cell = row.createCell(8);  cell.setCellValue(ndDTO.getNomProducto());								
								cell = row.createCell(9);  cell.setCellValue(ndDTO.getAreaAplicada().doubleValue());
								cell = row.createCell(10); cell.setCellValue(ndDTO.getConsecutivo().longValue());
								cell = row.createCell(11); cell.setCellValue(ndDTO.getIdInsumos().longValue());
							...
							file = new DefaultStreamedContent(stream, "application/xlsx", "LaboresPorInsumos.xlsx");
	...
	public List<CostosInsumosDetalladoDTO> getDataDetalle() {
		return dataDetalle;
	}

	public void setDataDetalle(List<CostosInsumosDetalladoDTO> dataDetalle) {
