...
@ManagedBean
@ViewScoped
public class ListarLaboresPorMaquinaView implements Serializable {
	private static final long serialVersionUID = 1L;
	@SuppressWarnings("unused")
	private static final Logger log = LoggerFactory.getLogger(ListarLaboresPorMaquinaView.class);

	@ManagedProperty(value = "#{BusinessDelegatorView}")
	private IBusinessDelegatorView businessDelegatorView;
	private List<HorasMaquinaDetalladoDTO> dataDetalle;
	...
	public ListarLaboresPorMaquinaView() {
		super();
	}
	...
	public void listarPlanillaNominaDetallada() {
		try {
			...
			if (compania != null && fechaInicial != null && fechaFinal != null) {
				...
				dataDetalle = businessDelegatorView.consultarInformeHorasMaquinaDetallado(compania, fechaInicial, fechaFinal,
					"0", 1L, haciendasSeleccionadas, flagHacienda, "0", 1L, lotesSeleccionados, flagLote, laboresSeleccionadas,
					flagLabor, "0", 1L, "0", 1L, "0", 1L, "0", 1L, "0", 1L, equiposSeleccionados, flagEquipo, 0L);
			}

			double totalUnidades = 0;
			double totalCosto = 0;
			if (dataDetalle != null && dataDetalle.size() >= 0) {
				for (HorasMaquinaDetalladoDTO data1 : dataDetalle) {
				...
				try {
					...
					try {
						...
						if (data != null) {
							...
							cell = row.createCell(4);  cell.setCellStyle(style2); cell.setCellValue("VR. UNITARIO");
							...
							cell = row.createCell(8);  cell.setCellStyle(style2); cell.setCellValue("MAQUINA");
							cell = row.createCell(9);  cell.setCellStyle(style2); cell.setCellValue("CONSECUTIVO");
							cell = row.createCell(10); cell.setCellStyle(style2); cell.setCellValue("ID");
							...
							for (HorasMaquinaDetalladoDTO ndDTO : data) {
								...
								cell = row.createCell(2);  cell.setCellValue(ndDTO.getCodUdadMed());
								cell = row.createCell(3);  cell.setCellValue(ndDTO.getCantidadPago().doubleValue());
								cell = row.createCell(4);  cell.setCellValue(ndDTO.getValorUnitario().doubleValue());
								...
								cell = row.createCell(8);  cell.setCellValue(ndDTO.getNomEquipo());
								cell = row.createCell(9);  cell.setCellValue(ndDTO.getConsecutivo().longValue());
								cell = row.createCell(10); cell.setCellValue(ndDTO.getDatServicioDetId().longValue());
							...
							file = new DefaultStreamedContent(stream, "application/xlsx", "LaboresPorMaquina.xlsx");
	...
	public List<HorasMaquinaDetalladoDTO> getDataDetalle() {
		return dataDetalle;
	}

	public void setDataDetalle(List<HorasMaquinaDetalladoDTO> dataDetalle) {
