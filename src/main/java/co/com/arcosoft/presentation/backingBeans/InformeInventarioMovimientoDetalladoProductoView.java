...
@ManagedBean
@ViewScoped
public class InformeInventarioMovimientoDetalladoProductoView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(InformeInventarioMovimientoDetalladoProductoView.class);
	...
	/*****************************/
	public InformeInventarioMovimientoDetalladoProductoView() {
		super();
	}
	...
	public void consultaSaldoProducto() {
		try {
			...
			if(txtAlmacenId_Almacen2.getValue()!=null) {
				almacenId = txtAlmacenId_Almacen2.getValue().toString();
						flagAlmacen = 0L;
			}
		 
			if(productoId!=null && fechaFinal!=null) {
				dataSaldoProducto = businessDelegator2View.pr_inventario_detalle( compania,  fechaFinalDateRegistro,  fechaFinal,
						"0",  1L, 
						productoId,  flagProducto, 
						almacenId ,  flagAlmacen, 
						 "0",  1L,0l,0l,tipoProducto,"0"
						);
				if(dataSaldoProducto!=null && dataSaldoProducto.size()>=0) {
					for( ConsultaCompraProductosDTO data1 : dataSaldoProducto) {
						cantidadEntrada   += data1.getCantidadEntrada().doubleValue() ;
						 cantidadSalida += data1.getCantidadSalida().doubleValue() ;
						 costoEntrada   += data1.getCostoEntrada().doubleValue() ;
						 costoSalida += data1.getCostoSalida().doubleValue() ;
						}
					 saldoTotal += cantidadEntrada - cantidadSalida ;
					 valorTotal +=  costoEntrada -costoSalida ;
					 if(saldoTotal>=0.1) {
						 txtCantidadEntrada.setValue(cantidadEntrada);
						 txtCantidadSalida.setValue(cantidadSalida);
						 txtSaldoTotal.setValue(saldoTotal);
						 txtValorTotalAcumulado.setValue(valorTotal); 
					 }else {
						 txtSaldoTotal.setValue(0.0);
						 txtValorTotalAcumulado.setValue(0.0);
						 txtCantidadEntrada.setValue(cantidadEntrada);
						 txtCantidadSalida.setValue(cantidadSalida);
					 }
				}
					...
					FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Upps!",
							"Verifique que los campos de fechas,  productos,  tengan valores. "));
	...
	public void consultaProductoPorPeriodo() {
		try {
			...
			if(txtTipoProducto3.getValue()!=null) {
				 tipoProducto = FacesUtils.checkLong(txtTipoProducto3);	
			}
			...
			if(txtAlmacenId_Almacen3.getValue()!=null) {
				almacenId = txtAlmacenId_Almacen3.getValue().toString();
						flagAlmacen = 0L;
			}
			...
			if(productoId!=null & fechaInicial != null & fechaFinal != null) {
				dataProductoPeriodo = businessDelegator2View.pr_inventario_detalle( compania,  fechaInicial,  fechaFinal,
						"0",  1L, 
						productoId,  flagProducto, 
						almacenId ,  flagAlmacen, 
						conceptoKardexSeleccionadas,  flagConceptoKardex,0l,0l,tipoProducto,"0"
						);
			}else {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Upps!",
						"Verifique que los campos de fechas,  productos,  tengan valores. "));
			...
			try {
				...
				for (Producto producto2 : lista) {
					selectProductoId[i] = new SelectItem(producto2.getProductoId(),
							...
							producto2.getCodigo() + "-" + producto2.getNombre() +"-"+producto2.getInfoAdicional()+"-"+producto2.getInfoAdicional2());
					i++;
	...
	public SelectItem[] getSelectProductoId3() {
		...
		if (productoId3 == null || productoId3.size() == 0) {
			...
			try {
				...
				int i = 0;
				for (Producto producto2 : lista) {
					selectProductoId3[i] = new SelectItem(producto2.getProductoId(),
