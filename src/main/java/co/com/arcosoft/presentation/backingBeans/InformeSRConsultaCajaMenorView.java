...
@ManagedBean
@ViewScoped
public class InformeSRConsultaCajaMenorView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(InformeSRConsultaCajaMenorView.class);
	...
	private List<ConsultaCajaMenorDTO> data;
	...
	private ConsultaCajaMenorDTO selectedDatServRealizadosEquipo;
	private List<ConsultaCajaMenorDTO> selectedDatServRealizadosEquipo2;
	

	private List<String> selectedCajas;
	private List<CajaMenor> cajas;
	private InputNumber txtValorTotal;
	
	public InformeSRConsultaCajaMenorView() {
		super();
	}
	...
	public void consultaCajaMenor() throws NumberFormatException, Exception {
		...
		String cajasSeleccionadas = "";
		if (selectedCajas != null && selectedCajas.size() > 0) {
			cajasSeleccionadas = selectedCajas.get(0);
			flagCajas = 0L;
			for (int i = 1; i < selectedCajas.size(); i++) {
				cajasSeleccionadas += "," + selectedCajas.get(i);
			}
		}
		...
		if (compania != null) {
			try {
				...
				List<ConsultaCajaMenorDTO> data = null;
				...
				data = businessDelegator2View.pr_caja_menor(compania, fechaInicial, fechaFinal, cajasSeleccionadas,flagCajas,
						equiposSeleccionadas, flagEquipo, laboresSeleccionadas, flagLabor, documento);
				...
				try {
					...
					if (data != null) {
						...
						cell = row.createCell(3	);	cell.setCellValue("COD CAJA");	cell.setCellStyle(style2);	
						cell = row.createCell(4	);	cell.setCellValue("NOM CAJA");	cell.setCellStyle(style2);	
						cell = row.createCell(5	);	cell.setCellValue("DOCUMENTO");	cell.setCellStyle(style2);	
						
						cell = row.createCell(6	);	cell.setCellValue("DETALLE CAJA");	cell.setCellStyle(style2);	
						cell = row.createCell(7	);	cell.setCellValue("VR. TOTAL");	cell.setCellStyle(style2);	
						
						cell = row.createCell(8	);	cell.setCellValue("COD EQUIPO");	cell.setCellStyle(style2);	
						cell = row.createCell(9	);	cell.setCellValue("NOM EQUIPO");	cell.setCellStyle(style2);	
						cell = row.createCell(10	);	cell.setCellValue("GASTO");	cell.setCellStyle(style2);	
						cell = row.createCell(11	);	cell.setCellValue("ID. PROVEEDOR");	cell.setCellStyle(style2);	
						cell = row.createCell(12	);	cell.setCellValue("PROVEEDOR");	cell.setCellStyle(style2);	
						
						cell = row.createCell(13	);	cell.setCellValue("FACTURA");	cell.setCellStyle(style2);	
						cell = row.createCell(14	);	cell.setCellValue("CENTRO COSTO");	cell.setCellStyle(style2);	
						
						
					
						int pos_i = 8;

						for (ConsultaCajaMenorDTO ndDTO : data) {
							...
							cell = row.createCell(3	);	cell.setCellValue(ndDTO.getCod_caja_menor());	
							cell = row.createCell(4	);	cell.setCellValue(ndDTO.getNom_caja_menor());	
							cell = row.createCell(5	);	cell.setCellValue(ndDTO.getConsecutivo().doubleValue());	
							cell = row.createCell(6	);	cell.setCellValue(ndDTO.getDetalle_nota());	
							cell = row.createCell(7	);	cell.setCellValue(ndDTO.getValor().doubleValue());	
							
							cell = row.createCell(8	);	cell.setCellValue(ndDTO.getCod_equipo());	
							cell = row.createCell(9	);	cell.setCellValue(ndDTO.getNom_equipo());	
							cell = row.createCell(10	);	cell.setCellValue(ndDTO.getNomLabor());	
							
							cell = row.createCell(11	);	cell.setCellValue(ndDTO.getCodProveedor());	
							cell = row.createCell(12	);	cell.setCellValue(ndDTO.getNomProveedor());	
							
							cell = row.createCell(13	);	cell.setCellValue(ndDTO.getNumFactura());	
							cell = row.createCell(14	);	cell.setCellValue(ndDTO.getCentroCosto());	
						...
						file = new DefaultStreamedContent(stream, "application/xlsx", "MttoCajaMenor.xlsx");
	...
	public void consultaCajaMenorEditable() {
		try {
			...
			if(documento !=null) {
				...
				data = businessDelegator2View.pr_caja_menor(compania, fechaFinalDateRegistro, date, "0",flagCajas,
						"0", flagEquipo, "0", flagLabor, documento);
			
				 if(data !=null & data.size()>0) {
					for( ConsultaCajaMenorDTO data1 : data) {
	...
	public ConsultaCajaMenorDTO getSelectedDatServRealizadosEquipo() {
		return selectedDatServRealizadosEquipo;
	}

	public void setSelectedDatServRealizadosEquipo(
			ConsultaCajaMenorDTO selectedDatServRealizadosEquipo) {
	...
	public List<ConsultaCajaMenorDTO> getSelectedDatServRealizadosEquipo2() {
		return selectedDatServRealizadosEquipo2;
	}

	public void setSelectedDatServRealizadosEquipo2(
			List<ConsultaCajaMenorDTO> selectedDatServRealizadosEquipo2) {
		this.selectedDatServRealizadosEquipo2 = selectedDatServRealizadosEquipo2;
	}


	public void setData(List<ConsultaCajaMenorDTO> data) {
		this.data = data;
	}
	...
	public List<ConsultaCajaMenorDTO> getData() {
		return data;
	}
	...
	public List<String> getSelectedCajas() {
		return selectedCajas;
	}

	public void setSelectedCajas(List<String> selectedCajas) {
		this.selectedCajas = selectedCajas;
	}

	public List<CajaMenor> getCajas() {

		if (cajas == null || cajas.size() == 0) {

			cajas = new ArrayList<CajaMenor>();

			try {
				cajas = businessDelegator2View.getCajaMenor();
			} catch (Exception e) {
				FacesUtils.addErrorMessage(e.getMessage());
				e.printStackTrace();
			}
	...
	public void setCajass(List<CajaMenor> cajas) {
		this.cajas = cajas;
	}
