...
@ManagedBean
@ViewScoped
public class DatAnaLaboratorioView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(DatAnaLaboratorioView.class);
	...
	private SelectOneMenu txtAnaLaboratorioIdConsulta;
	SelectItem[] selectTipoAnalisisLaboratorioConsulta;
	private List<AnaLaboratorio> tipoAnalisisConsulta;

	private Long idDatAnaLaboratorio_calcular;

	@Resource
	private SessionFactory sessionFactory;

	public DatAnaLaboratorioView() {
		super();
	}
	...
	public String action_clear() throws ParseException {
		...
		selectedDatAnaLaboratorio = null;
	//	RequestContext.getCurrentInstance().reset(":dialogDatAnaLaboratorio :frm");
		...
		if (txtFechaAnalisis != null) {
			Date date = new Date();
			txtFechaAnalisis.setValue(date);
	...
	public List<ListaVariablesAnaLaboratorioDTO> agregarNuevasVariables(Long datAnaLaboratorioId) {

		String consulta = "call pr_lista_variables_faltantes_laboratorio (:datAnaLaboratorioId)";
		...
		parameters.addValue("datAnaLaboratorioId", datAnaLaboratorioId);
		
		List<ListaVariablesAnaLaboratorioDTO> list = new ArrayList<ListaVariablesAnaLaboratorioDTO>();
		
		List<Map<String, Object>> rows = namedParameterJdbcTemplate.queryForList(consulta, parameters);
        
		
		for (Map row : rows) {
			
			ListaVariablesAnaLaboratorioDTO dto = new ListaVariablesAnaLaboratorioDTO();
			
			dto.setIdAnalisis(BigInteger.valueOf((Long) row.get("id_analisis")));
			dto.setNombreAnalisis((String) row.get("nom_analisis"));
			dto.setIdVariable(BigInteger.valueOf((Long) row.get("id_variable")));
			dto.setCodVariable((String) row.get("cod_variable"));
			dto.setNomVariable((String) row.get("nom_variable"));
			dto.setOrden(BigInteger.valueOf((Long)row.get("orden_digitacion")));
			dto.setTipoVariable((String) row.get("tipo_variable"));
			dto.setFormula((String) row.get("formula"));
			dto.setCompaniaId(BigInteger.valueOf((Long) row.get("compania")));
			
			dto.setValorVariableDia((BigDecimal) row.get("valor_variable_dia"));
			dto.setValorVariableHora6((BigDecimal) row.get("valor_hora6"));
			dto.setValorVariableHora7((BigDecimal) row.get("valor_hora7"));
			dto.setValorVariableHora8((BigDecimal) row.get("valor_hora8"));
			dto.setValorVariableHora9((BigDecimal) row.get("valor_hora9"));
			dto.setValorVariableHora10((BigDecimal) row.get("valor_hora10"));
			dto.setValorVariableHora11((BigDecimal) row.get("valor_hora11"));
			dto.setValorVariableHora12((BigDecimal) row.get("valor_hora12"));
			dto.setValorVariableHora13((BigDecimal) row.get("valor_hora13"));
			dto.setValorVariableHora14((BigDecimal) row.get("valor_hora14"));
			dto.setValorVariableHora15((BigDecimal) row.get("valor_hora15"));
			dto.setValorVariableHora16((BigDecimal) row.get("valor_hora16"));
			dto.setValorVariableHora17((BigDecimal) row.get("valor_hora17"));
			dto.setValorVariableHora18((BigDecimal) row.get("valor_hora18"));
			dto.setValorVariableHora19((BigDecimal) row.get("valor_hora19"));
			dto.setValorVariableHora20((BigDecimal) row.get("valor_hora20"));
			dto.setValorVariableHora21((BigDecimal) row.get("valor_hora21"));
			dto.setValorVariableHora22((BigDecimal) row.get("valor_hora22"));
			dto.setValorVariableHora23((BigDecimal) row.get("valor_hora23"));
			dto.setValorVariableHora0((BigDecimal) row.get("valor_hora0"));
			dto.setValorVariableHora1((BigDecimal) row.get("valor_hora1"));
			dto.setValorVariableHora2((BigDecimal) row.get("valor_hora2"));
			dto.setValorVariableHora3((BigDecimal) row.get("valor_hora3"));
			dto.setValorVariableHora4((BigDecimal) row.get("valor_hora4"));
			dto.setValorVariableHora5((BigDecimal) row.get("valor_hora5"));
			
			dto.setHoraDigitacion((String) row.get("hora_digitacion"));
			dto.setFrecuenciaDigitacion((String) row.get("frecuencia_digitacion"));
			dto.setDat_ana_lab_id(BigInteger.valueOf((Long)row.get("dat_ana_lab_id")));
	...
	public String action_edit(ActionEvent evt) {
		...
		try {
			...
			if (lista != null && lista.size() > 0) {
				...
				Long idAnalisis = entity.getAnaLaboratorio().getAnaLabId();
				AnaLaboratorio l = businessDelegatorView.getAnaLaboratorio(idAnalisis);
				txtHoraDigitacion.setValue(entity.getHoraDigitacion());

				if (l.getFrecuenciaDigitacion().equals("Horaria")) {
					txtHoraDigitacion.setDisabled(false);
				}
				if (l.getFrecuenciaDigitacion().equals("Diaria")) {
				...
				dataVariables = agregarNuevasVariables(idDatAnaLaboratorio);
				
				if(dataVariables !=null && dataVariables.size()>0) {
					businessDelegatorView.updateDatAnaLaboratorio(entity, null, dataVariables);
				}
	...
	public String action_create() {
		try {
			...
			analisisExiste = businessDelegator2View.pr_validar_existencia_analisis_lab(compania, fecha, hora, idAnalisis);
			if (analisisExiste != null && analisisExiste.size() > 0) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Upps! Ya existe un análisis digitado para esa fecha y hora.", " Verifique por favor."));
				...
				entity.setAnaLaboratorio(
						(FacesUtils.checkLong(txtAnaLaboratorioId_AnaLaboratorio) != null) ? businessDelegatorView
								.getAnaLaboratorio(FacesUtils.checkLong(txtAnaLaboratorioId_AnaLaboratorio)) : null);
				...
				idDatAnaLaboratorio_calcular = idDatAnaLaboratorio;
	...
	public void listarAnalisis() {
		try {
			...
			Long idAnalisis = null;
			idAnalisis = FacesUtils.checkLong(txtAnaLaboratorioIdConsulta);
			data = businessDelegatorView.getDataDatAnaLaboratorioByTipoAnalisis(idAnalisis);
	...
	/*
	 * public List<DatAnaLaboratorioDTO> getData() { try { if (data == null) {
	 * 
	 * data = businessDelegatorView.getDataDatAnaLaboratorio();
	 * 
	 ...
	 * 
	 * 
	 * public void setData(List<DatAnaLaboratorioDTO> data) { this.data = data; }
	 */
	...
	public void setData(List<DatAnaLaboratorioDTO> data) {
		this.data = data;
	}
	...
	/**
	 * @param dataVariables the dataVariables to set
	 */
	...
	/**
	 * @param txtVariableDia the txtVariableDia to set
	 */
	...
	/**
	 * @param txtHora0000 the txtHora0000 to set
	 */
	...
	/**
	 * @param txtHora0100 the txtHora0100 to set
	 */
	...
	/**
	 * @param txtHora0200 the txtHora0200 to set
	 */
	...
	/**
	 * @param txtHora0300 the txtHora0300 to set
	 */
	...
	/**
	 * @param txtHora0400 the txtHora0400 to set
	 */
	...
	/**
	 * @param txtHora0500 the txtHora0500 to set
	 */
	...
	/**
	 * @param txtHora0600 the txtHora0600 to set
	 */
	...
	/**
	 * @param txtHora0700 the txtHora0700 to set
	 */
	...
	/**
	 * @param txtHora0800 the txtHora0800 to set
	 */
	...
	/**
	 * @param txtHora0900 the txtHora0900 to set
	 */
	...
	/**
	 * @param txtHora1000 the txtHora1000 to set
	 */
	...
	/**
	 * @param txtHora1100 the txtHora1100 to set
	 */
	...
	/**
	 * @param txtHora1200 the txtHora1200 to set
	 */
	...
	/**
	 * @param txtHora1300 the txtHora1300 to set
	 */
	...
	/**
	 * @param txtHora1400 the txtHora1400 to set
	 */
	...
	/**
	 * @param txtHora1500 the txtHora1500 to set
	 */
	...
	/**
	 * @param txtHora1600 the txtHora1600 to set
	 */
	...
	/**
	 * @param txtHora1700 the txtHora1700 to set
	 */
	...
	/**
	 * @param txtHora1800 the txtHora1800 to set
	 */
	...
	/**
	 * @param txtHora1900 the txtHora1900 to set
	 */
	...
	/**
	 * @param txtHora2000 the txtHora2000 to set
	 */
	...
	/**
	 * @param txtHora2100 the txtHora2100 to set
	 */
	...
	/**
	 * @param txtHora2200 the txtHora2200 to set
	 */
	...
	/**
	 * @param txtHora2300 the txtHora2300 to set
	 */
	...
	/**
	 * @param entityDatAnaLaboratorioDet the entityDatAnaLaboratorioDet to set
	 */
	...
	/**
	 * @param selectedValor the selectedValor to set
	 */
	...
	/**
	 * @param selectedValorAnalisis the selectedValorAnalisis to set
	 */
	...
	public void actionHoraDigitacion() {

		Long idAnalisis = null;

		if (FacesUtils.checkLong(txtAnaLaboratorioId_AnaLaboratorio) != null) {
			if (!txtAnaLaboratorioId_AnaLaboratorio.getValue().equals("")) {
				idAnalisis = Long.parseLong(txtAnaLaboratorioId_AnaLaboratorio.getValue().toString());
	...
	public void onCellEditEventos(CellEditEvent evt) throws Exception {
		...
		ScriptEngine interprete = manager.getEngineByName("js");
		Bindings scope = interprete.createBindings();
		Long datAnaLaboratorioId = selectedValorAnalisis.getDatAnaLabId_DatAnaLaboratorio().longValue();
	...
	public void action_calcularValoresVariables() {
		...
		try {
			...
			if (dataValor != null && dataValor.size() > 0) {
				...
				// Id del Analisis al momento de crear el registro
				Long idDatAnaLaboratorio = dataValor.get(0).getDatAnaLabId_DatAnaLaboratorio();
				List<DatAnaLaboratorioDetDTO> detAna = businessDelegatorView
						.getDataDatAnaLaboratorioDetByAnalisis(idDatAnaLaboratorio);

				Long idDatAnalisis = null;

				if (detAna != null && detAna.size() > 0) {

					for (DatAnaLaboratorioDetDTO data : detAna) {

						// Long valorVarId =
						// selectedValorAnalisis.getDatAnaLaboratorioDetId().longValue();
						Long valorVarId = data.getDatAnaLaboratorioDetId().longValue();
						// Long variable =
						// selectedValorAnalisis.getVariableLabId_VariableLab().getVariableLabId().longValue();
						Long variable = data.getVariableLabId_VariableLab().getVariableLabId();
						// Long idDatAnalisis =
						// selectedValorAnalisis.getDatAnaLabId_DatAnaLaboratorio();
						idDatAnalisis = data.getDatAnaLabId_DatAnaLaboratorio();
						...
						if (tipoVariable.equals("Consulta_Sql")) {
							...
							namedParameterJdbcTemplate = new NamedParameterJdbcTemplate(
									DriverManagerDataSourceUtils.getDatasource());
						...
						if (tipoVariable.equals("Formula")) {
							...
							for (int j = 0; j < variablesEncontradas.length; j++) {
								...
								if (variablesEncontradas[j] != null) {
									...
									listaVar = businessDelegatorView.findByCriteriaInVariableLab(condicionVar, null,
											null);

									if (listaVar != null && listaVar.size() > 0) {

										Object[] condicionDet = { "variableLab", true,
												listaVar.get(0).getVariableLabId().longValue(), "=",
												"datAnaLaboratorio.datAnaLabId", true, idAnalisis, "=" };
										lista = businessDelegatorView.findByCriteriaInDatAnaLaboratorioDet(condicionDet,
												null, null);
							...
							if (ejecutaFormula) {
								...
								double redondeado = new BigDecimal(valorFinal).setScale(3, RoundingMode.HALF_EVEN)
										.doubleValue();
					...
					selectedValorAnalisis = null;
					dataValor = null;
					dataValor = businessDelegatorView.getDataDatAnaLaboratorioDetByAnalisis(idDatAnalisis);
			...
			entity = null;
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"Upps! El código SQL/Fórmula asociado a la variable tiene errores", "Verifique la consulta."));
	...
	public String actionCalcularVariable(ActionEvent evt) {
		...
		try {
			...
			if (tipoVariable.equals("Consulta_Sql")) {
				...
				valorResultado = Double.parseDouble(result);
				// String lista = jdbcTemplate.queryForObject(consulta, namedParameters,
				// String.class);
			...
			if (tipoVariable.equals("Formula")) {
				 ...
				 * FacesMessage(FacesMessage.SEVERITY_WARN, "La variable " + vlab.getCodigo() +
				 * " es formulada " + vlab.getFormula().toString() + " con periodicidad " +
				 * vlab.getPeriocidadVariable(), ""));
				 */
				String formula = vlab.getFormula().toString();
				...
				if (ejecutaFormula) {
					...
					Double valorFinal = Double.parseDouble(resultadoFormula);
					// calcular formula y grabar en la posición seleccionada
					double redondeado = new BigDecimal(valorFinal).setScale(3, RoundingMode.HALF_EVEN).doubleValue();
	...
	public String actionDeleteMovimientos(ActionEvent evt) {
		...
		try {
			...
			Long iD = selectedDatAnaLaboratorio.getDatAnaLabId().longValue();
			Object[] condicion = { "datAnaLabId", true, iD, "=" };
			List<DatAnaLaboratorio> lista = (iD != null)
					? businessDelegatorView.findByCriteriaInDatAnaLaboratorio(condicion, null, null)
					: null;
			...
			if (lista != null && lista.size() > 0) {
				...
				Long borrarDetalle = businessDelegator2View.pr_borrar_dat_ana_laboratorio_det(iD);
				Long borrarCompra = businessDelegator2View.pr_borrar_dat_ana_laboratorio(iD);
	...
	public SelectItem[] getSelectTipoAnalisisLaboratorioConsulta() {

		if (tipoAnalisisConsulta == null || tipoAnalisisConsulta.size() == 0) {
			...
			try {
				...
				List<AnaLaboratorio> lista = businessDelegatorView.findByCriteriaInAnaLaboratorio(condicion, null,
						null);
				selectTipoAnalisisLaboratorioConsulta = new SelectItem[lista.size()];

				int i = 0;
				for (AnaLaboratorio tipoAnalisis : lista) {
					selectTipoAnalisisLaboratorioConsulta[i] = new SelectItem(tipoAnalisis.getAnaLabId(),
							tipoAnalisis.getNombre());
					i++;
		...
		return selectTipoAnalisisLaboratorioConsulta;
	}

	public void setSelectTipoAnalisisLaboratorioConsulta(SelectItem[] selectTipoAnalisisLaboratorioConsulta) {
		this.selectTipoAnalisisLaboratorioConsulta = selectTipoAnalisisLaboratorioConsulta;
	}

	public SelectOneMenu getTxtAnaLaboratorioIdConsulta() {
		return txtAnaLaboratorioIdConsulta;
	}

	public void setTxtAnaLaboratorioIdConsulta(SelectOneMenu txtAnaLaboratorioIdConsulta) {
		this.txtAnaLaboratorioIdConsulta = txtAnaLaboratorioIdConsulta;
	}

	public Long getIdDatAnaLaboratorio_calcular() {
		return idDatAnaLaboratorio_calcular;
	}

	public void setIdDatAnaLaboratorio_calcular(Long idDatAnaLaboratorio_calcular) {
		this.idDatAnaLaboratorio_calcular = idDatAnaLaboratorio_calcular;
	}
