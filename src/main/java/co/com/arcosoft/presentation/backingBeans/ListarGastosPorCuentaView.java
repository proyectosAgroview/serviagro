...
@ManagedBean
@ViewScoped
public class ListarGastosPorCuentaView implements Serializable {
	private static final long serialVersionUID = 1L;
	@SuppressWarnings("unused")
	private static final Logger log = LoggerFactory.getLogger(ListarGastosPorCuentaView.class);
	...
	private List<String> selectedCcontable;
	private List<CuentaContable> ccontable;
	
	private StreamedContent file = null;	
	private String visible = "hidden";
	private String disableButton = "true";

	public ListarGastosPorCuentaView() {
		super();
	}
	...
	public void listarPlanillaNominaDetallada() {
		try {
			...
			Long flagCcontable = 1L;
			String ccontablesSeleccionadas = "0";
			if (selectedCcontable != null && selectedCcontable.size() > 0) {
				ccontablesSeleccionadas = selectedCcontable.get(0);
				flagCcontable = 0L;
				for (int i = 1; i < selectedCcontable.size(); i++) {
					ccontablesSeleccionadas += "," + selectedCcontable.get(i);
				}
			}
			...
			if (compania != null && fechaInicial != null && fechaFinal != null) {
				...
				dataDetalle = businessDelegatorView.consultarCostosIndirectos(compania, fechaInicial, fechaFinal,
					"0", 1L, "0", 1L, "0", 1L, "0", 1L, ccontablesSeleccionadas, flagCcontable);
			}
	...
	public List<String> getSelectedCcontable() {
		return selectedCcontable;
	}

	public void setSelectedCcontable(List<String> selectedCcontable) {
		this.selectedCcontable = selectedCcontable;
	}

	public List<CuentaContable> getCcontable() {

		if (ccontable == null || ccontable.size() == 0) {

			try {
				ccontable = businessDelegatorView.getCuentaContable();
			} catch (Exception e) {
	...
	public void setCcontable(List<CuentaContable> ccontable) {
