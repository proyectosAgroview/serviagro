...
@ManagedBean
@ViewScoped
public class DatFacturaServiciosTercerosView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(DatFacturaServiciosTercerosView.class);
	...
	private SelectOneMenu txtClienteId;
	...
	private InputText txtDatFacturaServiciosTercerosId;
	...
	private List<DatFacturaServiciosTercerosDTO> data;
	private List<FacturaServiciosConsolidadosTercerosDTO> data2;
	private DatFacturaServiciosTercerosDTO selectedDatFacturaServiciosTerceros;
	private DatFacturaServiciosTerceros entity;

	private FacturaServiciosConsolidadosTercerosDTO selectFacturas;
	...
	private SelectOneMenu txtTipoOperacion;
	public DatFacturaServiciosTercerosView() {
		super();
	}

	public String action_new() throws NumberFormatException, Exception {
		action_clear();
		selectedDatFacturaServiciosTerceros = null;
		setShowDialog(true);
	...
	public String action_clear() throws NumberFormatException, Exception {
		...
		if (txtConsecutivo != null) {
			...
			txtConsecutivo.setValue(businessDelegator2View.consec_dat_factura_servicios_terceros(compania));
			txtConsecutivo.setDisabled(true);
		}
		if (txtTipoOperacion != null) {
			txtTipoOperacion.setValue(null);
			txtTipoOperacion.setDisabled(false);
		}
		...
		if (txtDatFacturaServiciosTercerosId != null) {
			txtDatFacturaServiciosTercerosId.setValue(null);
			txtDatFacturaServiciosTercerosId.setDisabled(false);
		}
	...
	public String action_edit(ActionEvent evt) {
		selectFacturas = (FacturaServiciosConsolidadosTercerosDTO) (evt.getComponent().getAttributes().get("selectFacturas"));
		try {

			Long id = selectFacturas.getDat_factura_servicios_id().longValue();
			Object[] condicion = { "datFacturaServiciosTercerosId", true, id, "=" };
			List<DatFacturaServiciosTerceros> lista = (id != null)
					? businessDelegator2View.findByCriteriaInDatFacturaServiciosTerceros(condicion, null, null)
					: null;
			...
			if (lista != null && lista.size() > 0) {
				...
				txtConsecutivo.setDisabled(true);
				txtDetalleFactura.setValue(entity.getDetalleFactura());
				txtDetalleFactura.setDisabled(false);

				txtFechaRegistro.setValue(entity.getFechaRegistro());
				txtFechaRegistro.setDisabled(false);

				txtPersEmprId.setValue(entity.getPersEmprByPersEmprId().getPersEmprId());
				txtPersEmprId.setDisabled(false);

				txtClienteId.setValue(entity.getPersEmprByClienteId().getPersEmprId());
				txtClienteId.setDisabled(false);

				txtValorFactura.setValue(entity.getValorFactura());
				txtValorFactura.setDisabled(false);

				txtDatFacturaServiciosTercerosId.setValue(entity.getDatFacturaServiciosTercerosId());
				txtDatFacturaServiciosTercerosId.setDisabled(true);
				...
				txtTipoOperacion.setValue(null);
				txtTipoOperacion.setValue(entity.getTipoOperacion());
				txtTipoOperacion.setDisabled(false);
	...
	public String action_save() {
		try {
			if ((selectedDatFacturaServiciosTerceros == null) && (entity == null)) {
				action_create();
			} else {
				action_modify();
			}
	...
	public String action_create() {
		try {
			entity = new DatFacturaServiciosTerceros();

			// Long datFacturaServiciosId =
			// FacesUtils.checkLong(txtDatFacturaServiciosTercerosId);
			Long compania = new Long(getCompaniaIdSession());
...
//            entity.setDatFacturaServiciosTercerosId(datFacturaServiciosId);
			...
			entity.setPersEmprByPersEmprId((FacesUtils.checkLong(txtPersEmprId) != null)
					? businessDelegatorView.getPersEmpr(FacesUtils.checkLong(txtPersEmprId))
					: null);

			entity.setPersEmprByClienteId((FacesUtils.checkLong(txtClienteId) != null)
					? businessDelegatorView.getPersEmpr(FacesUtils.checkLong(txtClienteId))
					: null);
			...
			entity.setTipoOperacion(FacesUtils.checkString(txtTipoOperacion));
			...
			if (fechaPin.before(fecha) && fechaMinimaPermitida.before(fechaPin)) {
				...
				// businessDelegatorView.actualizarNumFacturaEnDatServRDet(cadenaFactura,
				// num_factura);
				businessDelegator2View.saveDatFacturaServiciosTerceros(entity);
	...
	public String action_modify() {
		try {
			if (entity == null) {
				Long datFacturaServiciosId = new Long(
						selectedDatFacturaServiciosTerceros.getDatFacturaServiciosTercerosId());
				entity = businessDelegator2View.getDatFacturaServiciosTerceros(datFacturaServiciosId);
			}
			Long compania = new Long(getCompaniaIdSession());
			Long usuarioId = new Long(getUsuarioIdSession());
			Date date = new Date();
			...
			if (fechaPin.before(fecha) && fechaMinimaPermitida.before(fechaPin)) {
				...
				businessDelegator2View.updateDatFacturaServiciosTerceros(entity);
	...
	public String action_delete_datatable(ActionEvent evt) {
		try {
			...
			entity = businessDelegator2View.getDatFacturaServiciosTerceros(id);
			action_delete();
		} catch (Exception e) {
			FacesUtils.addErrorMessage(e.getMessage());
		}
	...
	public void action_delete() throws Exception {
		try {
			businessDelegator2View.deleteDatFacturaServiciosTerceros(entity);
	...
	public String action_delete_master() {
		try {
			Long datFacturaServiciosId = FacesUtils.checkLong(txtDatFacturaServiciosTercerosId);
			entity = businessDelegator2View.getDatFacturaServiciosTerceros(datFacturaServiciosId);
			action_delete();
		} catch (Exception e) {
			FacesUtils.addErrorMessage(e.getMessage());
	...
	public String action_modifyWitDTO(Long compania, Long consecutivo, Long datFacturaServiciosId,
		...
		try {
			...
			// renderManager.getOnDemandRenderer("DatFacturaServiciosTercerosView").requestRender();
			FacesUtils.addErrorMessage(e.getMessage());
			throw e;
		}
	...
	public InputText getTxtDatFacturaServiciosTercerosId() {
		return txtDatFacturaServiciosTercerosId;
	}

	public void setTxtDatFacturaServiciosTercerosId(InputText txtDatFacturaServiciosTercerosId) {
		this.txtDatFacturaServiciosTercerosId = txtDatFacturaServiciosTercerosId;
	}

	public List<DatFacturaServiciosTercerosDTO> getData() {
		try {
			if (data == null) {
				data = businessDelegator2View.getDataDatFacturaServiciosTerceros();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void setData(List<DatFacturaServiciosTercerosDTO> datFacturaServiciosDTO) {
		this.data = datFacturaServiciosDTO;
	}

	public DatFacturaServiciosTercerosDTO getSelectedDatFacturaServiciosTerceros() {
		return selectedDatFacturaServiciosTerceros;
	}

	public void setSelectedDatFacturaServiciosTerceros(DatFacturaServiciosTercerosDTO datFacturaServicios) {
		this.selectedDatFacturaServiciosTerceros = datFacturaServicios;
	}
	...
	public String action_saveAnularReg() {
		try {
			...
			if (entity == null) {
				Long id = new Long(selectedDatFacturaServiciosTerceros.getDatFacturaServiciosTercerosId().longValue());
				entity = businessDelegator2View.getDatFacturaServiciosTerceros(id);
			}
	...
	public String action_anularReg(ActionEvent evt) throws Exception {
		...
		action_clear();
		selectedDatFacturaServiciosTerceros = (DatFacturaServiciosTercerosDTO) (evt.getComponent().getAttributes()
				.get("selectedDatFacturaServiciosTerceros"));
		setShowDialog(true);
	...
	public DatFacturaServiciosTerceros getEntity() {
		return entity;
	}

	public void setEntity(DatFacturaServiciosTerceros entity) {
		this.entity = entity;
	}
	...
	public void listarFacturaServicios() {
		try {
			...
			if (fechaInicial != null && fechaFinal != null) {
				data2 = businessDelegator2View.pr_listar_factura_servicios_terceros(compania, fechaInicial, fechaFinal,
						documento , centCost);
			} else {
	...
	public List<FacturaServiciosConsolidadosTercerosDTO> getData2() {
		return data2;
	}

	public void setData2(List<FacturaServiciosConsolidadosTercerosDTO> data2) {
		this.data2 = data2;
	}
	...
	public SelectOneMenu getTxtClienteId() {
		return txtClienteId;
	}
	...
	public void setTxtClienteId(SelectOneMenu txtClienteId) {
	...
	public SelectOneMenu getTxtTipoOperacion() {
		return txtTipoOperacion;
	}

	public void setTxtTipoOperacion(SelectOneMenu txtTipoOperacion) {
