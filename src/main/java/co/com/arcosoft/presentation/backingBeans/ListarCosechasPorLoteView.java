...
@ManagedBean
@ViewScoped
public class ListarCosechasPorLoteView implements Serializable {
	private static final long serialVersionUID = 1L;
	@SuppressWarnings("unused")
	private static final Logger log = LoggerFactory.getLogger(ListarCosechasPorLoteView.class);
	...
	private List<CosechasLoteDTO> dataDetalle;
	...
	private InputNumber txtNumCortes;
	...
	public ListarCosechasPorLoteView() {
		super();
	}
	...
	public void listarPlanillaNominaDetallada() {
		try {
			...
			Long flagNumeroCosechas = 0L;
			fechaInicial = (FacesUtils.checkDate(txtFechaIni));
			fechaFinal = (FacesUtils.checkDate(txtFechaFin));
			flagNumeroCosechas = (FacesUtils.checkLong(txtNumCortes));			

			if (txtNumCortes.getValue().equals("") || flagNumeroCosechas == null ) {
			...
			if (compania != null && fechaInicial != null && fechaFinal != null) {
				...
				dataDetalle = businessDelegator2View.pr_pyg_lotes_cosechados(compania, fechaInicial, fechaFinal, "0", 1L, 
					haciendasSeleccionadas, flagHacienda, "0", 1L, lotesSeleccionados, flagLote, "0", 1L, flagNumeroCosechas, "0");
			}

			double totalCosto = 0;
			if (dataDetalle != null && dataDetalle.size() >= 0) {
				for (CosechasLoteDTO data1 : dataDetalle) {
		...
		try {
			...
			if (compania != null) {
				try {
					...
					List<CosechasLoteDTO> data = null;
					...
					filename = servletContext.getRealPath("") + File.separator + "informes" + File.separator
							+ "CosechasPorLote.xlsx";

					data = businessDelegator2View.pr_pyg_lotes_cosechados(compania, fechaInicial, fechaFinal, "0", 1L, 
						haciendasSeleccionadas, flagHacienda, "0", 1L, lotesSeleccionados, flagLote, "0", 1L, flagNumeroCosechas, "0");
					...
					try {
						...
						if (data != null) {
							...
							cell = row.createCell(0); cell.setCellStyle(style2); cell.setCellValue("LOTE");
							cell = row.createCell(1); cell.setCellStyle(style2); cell.setCellValue("NOMBRE LABOR");
							cell = row.createCell(2); cell.setCellStyle(style2); cell.setCellValue("FECHA");
							cell = row.createCell(3); cell.setCellStyle(style2); cell.setCellValue("UNIDAD");
							cell = row.createCell(4); cell.setCellStyle(style2); cell.setCellValue("CANTIDAD");
							cell = row.createCell(5); cell.setCellStyle(style2); cell.setCellValue("VR. UNITARIO");
							cell = row.createCell(6); cell.setCellStyle(style2); cell.setCellValue("SUBTOTAL");														
							cell = row.createCell(7); cell.setCellStyle(style2); cell.setCellValue("DOCUMENTO");
							cell = row.createCell(8); cell.setCellStyle(style2); cell.setCellValue("REGISTRO");

							int pos_i = 8;

							for (CosechasLoteDTO ndDTO : data) {

								row = sheet.createRow(pos_i); 
								/*cell = row.createCell(0); cell.setCellValue(ndDTO.getLoteA());
								...
								cell = row.createCell(3); cell.setCellValue(ndDTO.getCodUdadMedCos());
								cell = row.createCell(4); cell.setCellValue(ndDTO.getCantidadPago().doubleValue());
								cell = row.createCell(5); cell.setCellValue(ndDTO.getValorUnitario().doubleValue());
								cell = row.createCell(6); cell.setCellValue(ndDTO.getCostoTotal().doubleValue());
								cell = row.createCell(7); cell.setCellValue(ndDTO.getId().longValue());
								cell = row.createCell(8); cell.setCellValue(ndDTO.getIdDetalle().longValue());*/
							...
							file = new DefaultStreamedContent(stream, "application/xlsx", "CosechasPorLote.xlsx");
						...
						e.printStackTrace();
						context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "Error: " + e.getMessage()));
					}
	...
	public List<CosechasLoteDTO> getDataDetalle() {
		return dataDetalle;
	}

	public void setDataDetalle(List<CosechasLoteDTO> dataDetalle) {
	...
	public InputNumber getTxtNumCortes() {
		return txtNumCortes;
	}

	public void setTxtNumCortes(InputNumber txtNumCortes) {
