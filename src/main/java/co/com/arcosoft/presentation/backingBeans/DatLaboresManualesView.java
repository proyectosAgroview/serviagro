...
@ManagedBean
@ViewScoped
public class DatLaboresManualesView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(DatLaboresManualesView.class);
	...
	public String action_create() {
		try {
			...
			entity.setOrigenDatos("MANO DE OBRA");
	...
	public String actionDeleteMovimiento(ActionEvent evt) {
		...
		try {
			Long id = selectedDatPlanillaNominaLista.getPlanillaNominaId();
			Object[] condicion = { "planillaNominaId", true, id, "=" };
			List<DatPlanillaNomina> lista = (id != null)
					? businessDelegatorView.findByCriteriaInDatPlanillaNomina(condicion, null, null)
					: null;
			...
			if (lista != null && lista.size() > 0) {
					...
					Object[] condicionEliminarDetalle = { "planillaNominaId.planillaNominaId", true, id, "=" };
					List<DatPlanillaNominaDet> listaDetalle = (id != null)
							? businessDelegatorView.findByCriteriaInDatPlanillaNominaDet(condicionEliminarDetalle, null,
									null)
							: null;
					if (listaDetalle != null && listaDetalle.size() > 0) {
						for (DatPlanillaNominaDet entidad : listaDetalle) {
							businessDelegatorView.deleteDatPlanillaNominaDet(entidad);
						}

						Object[] condicionEliminarPpal = { "planillaNominaId", true, id, "=" };
						List<DatPlanillaNomina> listaPpal = (id != null)
								? businessDelegatorView.findByCriteriaInDatPlanillaNomina(condicionEliminarPpal, null,
										null)
								: null;
						if (listaPpal != null && listaPpal.size() > 0) {
							DatPlanillaNomina entidadPpal = listaPpal.get(0);
							businessDelegatorView.deleteDatPlanillaNomina(entidadPpal);
						}

						FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
								"Proceso exitoso!", "El movimiento de incentivo fue eliminado exitosamente!!!"));
					}else{
						DatPlanillaNomina entidadPpal = lista.get(0);
						businessDelegatorView.deleteDatPlanillaNomina(entidadPpal);
