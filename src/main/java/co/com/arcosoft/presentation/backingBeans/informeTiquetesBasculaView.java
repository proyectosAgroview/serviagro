...
@ManagedBean
@ViewScoped
public class informeTiquetesBasculaView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory
			.getLogger(informeTiquetesBasculaView.class);
	...
	private List<TiquetesBasculaDTO> data;
	...
	public informeTiquetesBasculaView() {
		super();
	}
	...
	public String exportToPyGExcel() {
		Long compania1 = FacesUtils.checkLong(txtCompania);
		...
		String selectedTiquet = "0";
		if (selectedTiquete != null && selectedTiquete.size() > 0) {
			selectedTiquet = selectedTiquete.get(0);
			flagTiquet = 0L;
			for (int i = 1; i < selectedTiquete.size(); i++) {
				selectedTiquet += "," + selectedTiquete.get(i);
			}
		}
		...
		if (selectedEquipo != null && selectedEquipo.size() > 0) {
			equipo = selectedEquipo.get(0);
			flagEquipo = 0L;
			for (int i = 1; i < selectedEquipo.size(); i++) {
				equipo += "," + selectedEquipo.get(i);
			}
		}
		...
		if (compania != null && fechaInicial !=null && fechaFinal !=null) {
			try {
				...
				List<TiquetesBasculaDTO> data=null;
				...
				filename = servletContext.getRealPath("") + File.separator + "informes" + File.separator
						+ "MovimientosBascula.xlsx";

				data = businessDelegator2View.consultarEstadoVehiculoPr(compania1, fechaInicial, fechaFinal, tipoMovimiento,
						tipoTransaccion, flagTipoTransaccion, equipo, flagEquipo, flagEstadoTiquete, estadoSeleccionado, selectedTiquet, flagTiquet,tiquetePesoNeto);
				...
				try {
					...
					if (data != null) {
						...
						Cell columna1 	=	row.createCell(0	);	columna1	.setCellValue(	"TIQUETE"	); 
						Cell columna2	=	row.createCell(1	);	columna2	.setCellValue(	"CONSECUTIVO"	);
						...
						Cell columna32	=	row.createCell(31	);	columna32	.setCellValue(	"TEMPERATURA"	);
						...
						Cell columna41	=	row.createCell(40	);	columna41	.setCellValue(	"ORIGEN/DESTINO"	);
						Cell columna42	=	row.createCell(41	);	columna42	.setCellValue(	"PRODUCTO"	);
						Cell columna43	=	row.createCell(42	);	columna43	.setCellValue(	"TIPO PESAJE TARA"	);
						Cell columna44	=	row.createCell(43	);	columna44	.setCellValue(	"TIPO PESAJE BRUTO"	);
						Cell columna45	=	row.createCell(44	);	columna45	.setCellValue(	"DAT_TRANS_PROD_ID"	);
						Cell columna46	=	row.createCell(45	);	columna46	.setCellValue(	"FECHA EN PORTERIA"	);
							
						columna1.setCellStyle(style1);
						...
						for(TiquetesBasculaDTO ndDTO: data){

							row = sheet.createRow(pos_i);
							
							Cell cell1 	=	row.createCell(0	);	cell1	.setCellValue(ndDTO.getConsecutivo_peso_neto().doubleValue());
							...
							Cell cell14	=	row.createCell(13	);	cell14	.setCellValue(ndDTO.getTrabajador());
							...
							Cell cell31	=	row.createCell(30	);	cell31	.setCellValue(ndDTO.getImpurezas());
							...
							Cell cell40	=	row.createCell(39	);	cell40	.setCellValue(ndDTO.getFecha_anulacion().toString());
							Cell cell41	=	row.createCell(40	);	cell41	.setCellValue(ndDTO.getLote());
							Cell cell42	=	row.createCell(41	);	cell42	.setCellValue(ndDTO.getProducto());
							Cell cell43	=	row.createCell(42	);	cell43	.setCellValue(ndDTO.getTipoPesajeTara());
							Cell cell44	=	row.createCell(43	);	cell44	.setCellValue(ndDTO.getTipoPesajeBruto());
							Cell cell45	=	row.createCell(44	);	cell45	.setCellValue(ndDTO.getDat_trans_prod_id().longValue());
							Cell cell46	=	row.createCell(45	);	cell46	.setCellValue(ndDTO.getFecha_vehiculo_porteria().toString().substring(0, 10));
						...
						file = new DefaultStreamedContent(stream, "application/xlsx", "MovimientosBascula.xlsx");
	...
	public List<TiquetesBasculaDTO> getData() {
		try {
			if (data == null) {
				data = businessDelegator2View.consultarEstadoVehiculoPr(1L, null,
