package co.com.arcosoft.dataaccess.api;
...
public class HibernateDaoImpl<T, PK extends Serializable> implements Dao<T, PK> {
	...
	@Override
	public Long countByCriteria(String whereCondition) {
		log.debug("Count " + entityClass.getName() + " " + whereCondition);

		String where = ((whereCondition == null) || (whereCondition.length() == 0)) ? "" : ("where " + whereCondition);
		final String queryString = "select count(*) from " + entityClass.getName() + " model " + where;
	...
	public List<T> findAll() {
		...
		try {
			...
			Long companiaUserId = getCompaniaIdSession();
			List<T> entitiesList = null;
			String whereCondition = null;

			try {

				if(entityClass.getField("compania") != null ){

					whereCondition = " compania" + "=" + companiaUserId;
					String whereCompania = ((whereCondition == null) || (whereCondition.length() == 0)) ? ""
							: ("where " + whereCondition);
					final String queryStringCompania = "select model from " + entityClass.getName() + " model " + whereCompania;
					entitiesList = sessionFactory.getCurrentSession().createQuery(queryStringCompania).list();
				...
				entitiesList = sessionFactory.getCurrentSession().createQuery(queryString).list();
			}
		...
		try {
			...
			try {
				...
				if(entityClass.getField("compania") != null ){
					...
					String whereCompania = " and compania = " + companiaUserId;
					String where = ((whereCondition == null) || (whereCondition.length() == 0)) ? ""
							: ("where " + whereCondition + whereCompania );
					final String queryStringCompania = "select model from " + entityClass.getName() + " model " + where;
					entitiesList = sessionFactory.getCurrentSession().createQuery(queryStringCompania).list();
	...
	@Override
	public List<T> findByCriteriaPage(String whereCondition, String sortColumnName, boolean sortAscending, int startRow,
			int maxResults) {
		log.debug("findPage " + entityClass.getName());

		// String where = ((whereCondition == nul) || (whereCondition.length()
		// == 0)) ? ""
		// : ("where " + whereCondition);
		...
		String where = ((whereCondition == null) || (whereCondition.length() == 0)) ? " where 1 = 1 " + whereCompania
				: ("where " + whereCondition + whereCompania);

		if ((sortColumnName != null) && (sortColumnName.length() > 0)) {
			try {
				String queryString = "select model from " + entityClass.getName() + " model " + where
						+ " order by model." + sortColumnName + " " + (sortAscending ? "asc" : "desc");
	...
	public Long getCompaniaIdSession() {
		...
		if (auth != null) {
			...
			login = auth.getName().toString();
			
			if(!login.equals("anonymousUser")){
				
				String condicion = "login = '" + login + "'";

				try {
	
					final String queryString = "select model from Usuarios model where " + condicion;
					List<T> entitiesList = sessionFactory.getCurrentSession().createQuery(queryString).list();
					u = (Usuarios) entitiesList.get(0);
				...
				if (u != null) {
					compania = u.getCompania().getCompaniaId();
				}
	
				if (compania == null) {
					throw new RuntimeException("No se encontró la compañía de usuario " + login);
				}
				
			}else{
				
				compania = 1L;
				log.info("iniciando sessión via RESTFul");
			...
			compania = 1L;
			log.info("El usuario no ha iniciado sessión por Login.."+ "/" + " iniciando sessión via RESTFul");
			//throw new RuntimeException("El usuario no ha iniciado sessión por Login"+ "/" + "iniciando session via WS con compañia por defecto");
		}
