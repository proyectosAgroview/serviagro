...
import co.com.arcosoft.dataaccess.dao.ISemanaDAO;
...
public class SemanaLogic implements ISemanaLogic {
	private static final Logger log = LoggerFactory.getLogger(SemanaLogic.class);
	...
	public List<Semana> getSemana() throws Exception {
		log.debug("finding all Semana instances");
		...
		try {
			...
			log.error("finding all Semana failed", e);
			throw new ZMessManager().new GettingException(ZMessManager.ALL + "Semana");
		} finally {
		}
	...
	public void saveSemana(Semana entity) throws Exception {
		log.debug("saving Semana instance");
		...
		try {
			...
			log.debug("save Semana successful");
		} catch (Exception e) {
			log.error("save Semana failed", e);
			throw e;
		} finally {
		}
	}
	...
	public void deleteSemana(Semana entity) throws Exception {
		log.debug("deleting Semana instance");

		if (entity == null) {
			throw new ZMessManager().new NullEntityExcepcion("Semana");
		}

		if (entity.getSemanaId() == null) {
			throw new ZMessManager().new EmptyFieldException("semanaId");
		}

		try {
			semanaDAO.delete(entity);

			log.debug("delete Semana successful");
		} catch (Exception e) {
			log.error("delete Semana failed", e);
			throw e;
		} finally {
		}
	}
	...
	public void updateSemana(Semana entity) throws Exception {
		log.debug("updating Semana instance");
		...
		try {
			...
			log.debug("update Semana successful");
		} catch (Exception e) {
			log.error("update Semana failed", e);
			throw e;
		} finally {
		}
	}
	...
	public List<SemanaDTO> getDataSemana() throws Exception {
		try {
			...
			for (Semana semanaTmp : semana) {
				...
				semanaDTO2.setSemanaId(semanaTmp.getSemanaId());
				semanaDTO2.setCodigo((semanaTmp.getCodigo() != null) ? semanaTmp.getCodigo() : null);
				semanaDTO2.setCompania((semanaTmp.getCompania() != null) ? semanaTmp.getCompania() : null);
				semanaDTO2.setEstado((semanaTmp.getEstado() != null) ? semanaTmp.getEstado() : null);
				semanaDTO2.setFechaCreacion(semanaTmp.getFechaCreacion());
				semanaDTO2.setFechaFinal(semanaTmp.getFechaFinal());
				semanaDTO2.setFechaInicial(semanaTmp.getFechaInicial());
				semanaDTO2.setFechaModificacion(semanaTmp.getFechaModificacion());
				semanaDTO2.setAnioFiscalId_AnioFiscal((semanaTmp.getAnioFiscal().getAnioFiscalId() != null)
						? semanaTmp.getAnioFiscal().getAnioFiscalId() : null);
	...
	public Semana getSemana(Long semanaId) throws Exception {
		log.debug("getting Semana instance");

		Semana entity = null;

		try {
			entity = semanaDAO.findById(semanaId);
		} catch (Exception e) {
			log.error("get Semana failed", e);
	...
	public List<Semana> findPageSemana(String sortColumnName, boolean sortAscending, int startRow, int maxResults)
		...
		try {
			entity = semanaDAO.findPage(sortColumnName, sortAscending, startRow, maxResults);
		} catch (Exception e) {
